...
void run_checks() 
{
  // create nested scope for the tests
  {
    ...
    assert(string(a.c_str()) == "First");
    assert(a.length() == 5);
    assert(string(b.c_str()) == "second");
    assert(b.length() == 6);
    assert(string(c.c_str()) == "s3c0nd");
    assert(string(d.c_str()) == "second");
    assert(string(e.c_str()) == "second");
    assert(b.c_str() == d.c_str()); // underlying strings are the same
    assert(b.c_str() != e.c_str()); // these are distinct
    assert(b.length() == 6);
    // capture memory usage from a, b, c, d, e
    assert(generic_string::unreleased_bytes() == 43);
    // destructors for a, b, c, d, e executed here (leaving nested scope)
  }

  // at this point, all allocated cow_strings should have been deallocated
