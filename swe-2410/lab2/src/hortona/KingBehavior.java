...
/**
 * Behavior for checkers that are kings
 */
public class KingBehavior extends MoveBehavior {

    @Override
    boolean isValidOrdinaryMove(Square square, Piece piece) {
        return square.getY() == piece.getY() - 1 && Math.abs(square.getX() - piece.getX()) == 1
                || square.getY() == piece.getY() + 1 && Math.abs(square.getX() - piece.getX()) == 1;
    }

    @Override
    Piece getCapturedPiece(Square square, Piece piece) {
        Piece ret;
        if (!(((square.getY() == piece.getY() + 2 || square.getY() == piece.getY() - 2)
