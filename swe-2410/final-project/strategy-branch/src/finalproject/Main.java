...
public class Main extends Application {
    private static final int TRAIN_LENGTH = 900;
    private static final int SLEEP_DURATION_MS = 14;
    private static final double PATH_NODE_WIDTH = 1.5;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        boolean good = false;
        while (!good) {
            path = PathGenerator.generatePath(2, (int)Math.pow(TWO_THOUSAND, 4),
        ...
        Thread resourceMonitorThread = new Thread(() -> {
            ...
            try {
                while (couldBeFalse) {
                    long memUsed = (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / TWO_THOUSAND;
                    String labelText = "Memory Used: " + memUsed + "MB";
                    Platform.runLater(() -> {
                        memUseLabel.setText(labelText);
                    });
                    Thread.sleep(TWO_THOUSAND / 2 /* ONE THOUSAND */);
                }
