...
public class TriangleAnalyzer {
    ...
    /**
     * This boolean will be true if all read sides are integers or false if any side
     * that is read in is a floating point number.
     */
    private boolean allIntegerSides = true;

    /**
     * This is the string representation for the triangle. It is the portion of the
     * textLine prior to the comment.
     */
    ...
    /**
     * This attribute indicates if the line entered has invalid syntax based upon
     * the specification.
     */
     ...
     *                 consists of 3 numbers (a, b, c) which are whitespace
     *                 separated
        ...
        try (Scanner sideScanner = new Scanner(triangleDefinitionString);) {
            ...
            for (int index = 0; index < 3; index++) {
                if (sideScanner.hasNextInt()) {
                    ...
                    triangleSides[index] = sideScanner.nextDouble();
                    if (triangleSides[index] > Integer.MAX_VALUE) {
                        invalidSyntax = true;
                    }
            ...
            if (sideScanner.hasNextDouble()) {
                // There should not be another side here.
                invalidSyntax = true;
            }

        } catch (InputMismatchException ex) {
            // This will occur if the input is damaged. When this occurs, set each side to
            // -1 to indicate they are invalid.
    ...
    /**
     * This method will analyze the triangle that has been setup. It should only be
     * called if we know the syntax is valid.
     */
    public void analyzeTriangle() {
        if (!invalidSyntax) {
            ...
            if (triangleSideCopy[0] <= 0 || triangleSideCopy[1] <= 0 || triangleSideCopy[2] <= 0) {
                ...
                if ((triangleSideCopy[0] == triangleSideCopy[1])
                        && (triangleSideCopy[1] == triangleSideCopy[2])) {
                ...
                // Now determine if it is a right triangle and adjust accordingly if it is. The
                // answer should be correct to at least 2 decimal places.
                if (Math.abs(Math.sqrt((triangleSideCopy[0] * triangleSideCopy[0])
                        + (triangleSideCopy[1] * triangleSideCopy[1]))
                        - Math.sqrt(triangleSideCopy[2] * triangleSideCopy[2])) < 0.1) {
                    this.rightTriangle = true;

                    if (triangleType == TriangleTypeEnum.ISOSCELES) {
                        triangleType = TriangleTypeEnum.RIGHT_ISOSCELES;
                    }
                    if (triangleType == TriangleTypeEnum.SCALENE) {
                        triangleType = TriangleTypeEnum.RIGHT_SCALENE;
                    }
                }
     ...
     * 
     * @return The three sides will be returned as an array of doubles. The sides
     *         should be in the entered order.
     */
    ...
    /**
     * This method will return the perimeter of the triangle if it is valid. If it
     * is not value, it will return -1.
     * 
     * @return The return will be the perimeter of the triangle. If the triangle is
     *         not valid, the return will be -1.
     */
    ...
    /**
     * This method will return an analysis string. If the syntax entered is valid,
     * it will include the definitions of the triangle,
     * whether it is valid and , if valid, the type of triangle it is. Additionally,
     * if valid, it
     * will return the perimeter. If there was a syntax error in the line, it will
     * indicate that there was a syntax error and return the raw entered line.
     * 
     * @return The return will be a string representing the validity of the
     *         triangle.
     */
    ...
    public String obtainAnalysisString() {
        ...
        if (!invalidSyntax) {
            ...
            retString.append(getTriangleDefinitionString());
            ...
            if (triangleType != TriangleTypeEnum.INVALID) {
                ...
                switch (triangleType) {
                    ...
                    case SCALENE:
                        ...
                        break;
                    case RIGHT_SCALENE:
                        retString.append(", right scalene");
                        break;
                    case RIGHT_ISOSCELES:
                        retString.append(", right isosceles");
                        break;
                }
                retString.append(" triangle with sides of length ");

                if (!getTriangleDefinitionString().contains(".") || allIntegerSides) {
