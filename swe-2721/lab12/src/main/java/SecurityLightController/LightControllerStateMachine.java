...
public class LightControllerStateMachine implements LightControllerCommandInterface {
    ...
    /**
     * This variable holds a reference to the light which is to be controlled by
     * this state
     * machine.
     */
    ...
    /**
     * This variable is a reference to the timer that is to be used to control the
     * blinking of lights. This timer is periodic in nature and will continue to
     * fire every x ms until shutdown.
     * activities.
     */
    private LightTimerInterface blinkTimer;

    /**
     * This variable is a reference to the timer that is to be used for one shot
     * activities. This timer will fire once after x ms have gone by.
     */
    ...
    /**
     * This method will set the given light that is controlled by this state
     * machine.
     * 
     * @param light This is the instance of the light that is controlled by this
     *              state machine.
     */
    ...
    /**
     * This method will set the instance of the timer that is to be used for motion
     * detection.
     *
    ...
    private void updateObservers(LightState state) {
        for (LightControllerStateMachineObserverInterface obs : this.observers) {
            // BUG FIX: updateLightState was called three times
    ...
    public void unsubscribe(LightControllerStateMachineObserverInterface obs) {
        // BUG FIX: add should be remove instead
     ...
     *
     * @param presentState This is the present state. It will be used to determine
     *                     which
     ...
     *
     * @param destinationState This is the new state. It will be used to determine
     *                         which
    ...
    public void signalAction(CommandActionEnum request) {
        ...
        final Object[][] stateTransitionTable = {
                // Note: The format for this object array is Present state, event, destination
                // state.
                // Lamp off Daylight Starting State.
                new Object[] { LightState.LAMP_OFF_DAYLIGHT, CommandActionEnum.MANUAL_SWITCH_ON,
                        LightState.LAMP_ON_FULL_BRIGHTNESS },
                new Object[] { LightState.LAMP_OFF_DAYLIGHT, CommandActionEnum.LIGHT_SENSOR_DARKENED,
                        LightState.LAMP_OFF_NIGHTIME },

                // Lamp on Full starting state.
                new Object[] { LightState.LAMP_ON_FULL_BRIGHTNESS, CommandActionEnum.MANUAL_SWITCH_OFF,
                        LightState.LAMP_OFF_DAYLIGHT },
                new Object[] { LightState.LAMP_ON_FULL_BRIGHTNESS, CommandActionEnum.LIGHT_SENSOR_DARKENED,
                        LightState.LAMP_ON_NIGHTIME_BRIGHTNESS },

                // Lamp off nighttime starting state.
                new Object[] { LightState.LAMP_OFF_NIGHTIME, CommandActionEnum.MOTION_DETECTED,
                        LightState.MOTION_DETECTED },
                new Object[] { LightState.LAMP_OFF_NIGHTIME, CommandActionEnum.MANUAL_SWITCH_ON,
                        LightState.LAMP_ON_NIGHTIME_BRIGHTNESS },
                new Object[] { LightState.LAMP_OFF_NIGHTIME, CommandActionEnum.LIGHT_SENSOR_LIGHTENED,
                        LightState.LAMP_OFF_DAYLIGHT },

                // BUG FIX: this state change should have existed but did not
                new Object[] { LightState.LAMP_OFF_NIGHTIME, CommandActionEnum.SECURITY_ALARM_TRIPPED,
                        LightState.INTRUSION_DETECTED },

                // Lamp on nighttime brightness starting state.
                new Object[] { LightState.LAMP_ON_NIGHTIME_BRIGHTNESS, CommandActionEnum.MANUAL_SWITCH_OFF,
                        LightState.LAMP_OFF_NIGHTIME },
                new Object[] { LightState.LAMP_ON_NIGHTIME_BRIGHTNESS, CommandActionEnum.LIGHT_SENSOR_LIGHTENED,
                        LightState.LAMP_ON_FULL_BRIGHTNESS },

                // Motion Detected starting state.
                new Object[] { LightState.MOTION_DETECTED, CommandActionEnum.SECURITY_ALARM_TRIPPED,
                        LightState.INTRUSION_DETECTED },
                new Object[] { LightState.MOTION_DETECTED, CommandActionEnum.MOTION_DETECTION_TIMER_EXPIRED,
                        LightState.LAMP_OFF_NIGHTIME },
                new Object[] { LightState.MOTION_DETECTED, CommandActionEnum.LIGHT_SENSOR_LIGHTENED,
                        LightState.LAMP_OFF_DAYLIGHT },

                // Intrusion detected starting state.
                new Object[] { LightState.INTRUSION_DETECTED, CommandActionEnum.ALARM_CLEARED,
                        LightState.LAMP_OFF_NIGHTIME },
        };
        ...
        // If there is a state change, first handle the exit conditions for the current
        // state and then handle the entry conditions for the next state.

        // BUG FIX: stateChange was assigned true instead of checking if it was true
        if (stateChange == true) {
            // Invoke exit actions.
            ...
            // If there is no state change, but we are in an intrusion detected state,
            // manage it.
    ...
    /**
     * This private method will be invoked to manage intrusion detection. In short,
     * it will handle the blinking of the light if an intrusion is detected.
     * 
