...
public class StockQuoteAnalyzer {
     ...
     * the class for making sounds. This may be null if the system is not intended
     * to make noise.
     */
    ...
    /**
     * This variable holds a reference to the source for stock quotes. Should not be
     * null once the system is setup.
     */
    private StockQuoteGeneratorInterface stockQuoteSource = null;

    /**
     * This is the most current quote for the stock. This will be null until a
     ...
     * @param stockQuoteSource This is the source that is to be used to obtain the
     *                         stock quotes.
     * @param audioPlayer      This is the audio player that will make sounds. It
     *                         may be null if no sounds are to be made.
     * @throws InvalidStockSymbolException Will be thrown if the symbol for the
     *                                     stock is invalid.
     * @throws StockTickerConnectionError  Will be thrown if the stock quote source
     *                                     is null.
     */
    ...
    /**
     * This method will obtain the stock symbol for the stock that is being
     * monitored here.
     * 
     ...
     * 
     * @throws StockTickerConnectionError Will be thrown if the routine is unable to
     *                                    obtain a current stock quote.
     */
    public void refresh() throws StockTickerConnectionError {
        ...
        try {
            ...
            // If for some reason, the source can not be connected to, we need to throw this
            // exception to indicate there is a problem.
            throw new StockTickerConnectionError("Unable to connect with Stock Ticker Source. Symbol:" + symbol);
            // Hmm. Commenting out this throw fixes my code quite a bit. I'll ship it like
            // this and fix it later... Bobbie SW...
        }
    }
    ...
    public void playAppropriateAudio() {
        if (audioPlayer != null) {
            try {
                if ((this.obtainPercentChangeSincePreviousClose() >= 2)
                        || (this.obtainChangeSincePreviousClose() > 1)) {
                    audioPlayer.playHappyMusic();
                } else if ((this.obtainPercentChangeSincePreviousClose() <= -2)
                        || (this.obtainChangeSincePreviousClose() < -1)) {
    ...
    /**
     * This method will obtain the status of the current stock, either RISING,
     * FALLING, or STABLE.
     * 
     * @return The return will be:
     *         RISING if the stock is up 2% or more since the last close or by more
     *         than $1.00 since the last close
     *         FALLING if the stock is down 2% or more since the last close or by
     *         more than $1.00 since the last close
     ...
     * @throws InvalidAnalysisState An InvalidAnalysisState Exception will be thrown
     *                              if a quote has not yet been retrieved.
     */
    ...
    /**
     * This method will obtain the current selling price for the given stock, which
     * is the last trade.
     * 
    ...
    public double obtainCurrentPrice() throws InvalidAnalysisState {
        ...
        return currentQuote.getLastTradingPrice();
    }
     ...
     *         be accurate to the nearest .01%. For example, 18.27% will be returned
     *         as 18.27.
     * @throws InvalidAnalysisState
     *                              An InvalidAnalysisState Exception will be thrown
     *                              if a quote
     *                              has not yet been retrieved.
     */
    public double obtainPercentChangeSincePreviousClose() throws InvalidAnalysisState {
        ...
        // Look at this cool code example I found ontline here.
        // https://www.geeksforgeeks.org/java-program-to-round-a-number-to-n-decimal-places/
        // I'm real proud of myself for not using stack overflow to figure this... And I
        // didn't even need to
        ...
        return Math.round((temp * 100) * Math.pow(10, 2)) / Math.pow(10, 2);
    }

    /**
     * This method will return the textual timestamp for the last update to this
     * stock.
     * 
     * @return The return will be the date and time of the last update to this stock
     *         quote in GMT.
     * @throws InvalidAnalysisState This exception will be thrown if the quote has
    ...
    public final String obtainLastUpdateTimestamp() throws InvalidAnalysisState {
        ...
        return currentQuote.getTimestamp();
    }
}
