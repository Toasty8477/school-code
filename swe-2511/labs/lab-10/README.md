...
***NOTE:*** the hashtag is an optional addition to a blog post and may not exist.
...
- Two required parameters - specified as a ***JSON string*** in the ***body*** of the request
  - post - String (required)
  - hashtag - String (optional)
    - ***NOTE:*** the hashtag is optional - Your server must be able to handle the case where the hashtag is not specified
- Response Format:
...
***NOTE:*** The hashtag is optional however if the hashtag is specified the server ***must*** validate that is a valid hashtag using the valid hashtag rules.
...
***NOTE:*** The posts should remain filtered after a new post is added.  If the new post hashtag does not match the filter, it should not be displayed.
...
***NOTE:*** The blog post filter is ***NOT*** displayed if there are no posts 
...
- (40 Points) Blog Server
  ...
  - Server performs parameter validation checking (post and hashtag values are validated)
...
- (50 Points) Client side - Interface
  ...
  - Error messages are clear and fit the error type
  - Correct display and clearing of error messages
  - Post Filter:
    - Filters on hashtag only
    - Filters using a case-insensitive substring
    - Filter is not displayed if there are no posts
    - Filter remains in effect when new posts are made