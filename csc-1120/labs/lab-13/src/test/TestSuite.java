...
class TestSuite {
    ...
    @BeforeEach
    public void setup() {
        arrayList = new OrderedList(new ArrayList<>());
        linkedList = new OrderedList(new LinkedList<>());
        trie = new Trie();
    }
    ...
    public void testSize() {
        ...
        trie.add("food");
        trie.add("town");
        Assertions.assertEquals(finalListSize, trie.size());
    }
    @Test
    @Order(1)
    public void testAdd() {
        // Add items to the list
        Assertions.assertTrue(trie.add("foo"));
        Assertions.assertTrue(trie.add("bar"));
        Assertions.assertTrue(trie.add("taco"));
        // Check for correct size
        Assertions.assertEquals(3, trie.size());
        // Try and add a duplicate
        Assertions.assertFalse(trie.add("foo"));
        Assertions.assertFalse(trie.add("Foo"));
        Assertions.assertFalse(trie.add("fOo"));
        Assertions.assertFalse(trie.add("foO"));
        // Try and add null or empty string
        Assertions.assertThrows(IllegalArgumentException.class, () -> trie.add(null));
        Assertions.assertThrows(IllegalArgumentException.class, () ->trie.add(""));
    }
    @Test
    @Order(2)
    ...
    public void testAllMatches() {
        ...
        trie.add("town");
        // Check for matches
        Assertions.assertArrayEquals(matchTest1, trie.allMatches("fo"));
        Assertions.assertArrayEquals(matchTest1, trie.allMatches("foo"));
        Assertions.assertArrayEquals(matchTest2, trie.allMatches("t"));
        Assertions.assertArrayEquals(emptyMatches, trie.allMatches(null));
    }
    @Test
    @Order(4)
    public void testBackingStructure() {
        Assertions.assertEquals("java.util.ArrayList", arrayList.getBackingClass());
        Assertions.assertEquals("java.util.LinkedList", linkedList.getBackingClass());
        Assertions.assertEquals("hortona.model.ListMap", trie.getBackingClass());
    }
}
